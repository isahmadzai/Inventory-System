...
import type { Category, Unit, Warehouse, Supplier, PaginatedData, ApiResponse } from '@/types';
...
export default function ProductCreatePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [catRes, unitRes, whRes, supRes] = await Promise.all([
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={form.name} onChange={e => handleChange('name', e.target.value)} placeholder="e.g., Widget A" />
                {errors.name && <p className="text-xs text-red-500">{errors.name}</p>}
              </div>
              <div className="space-y-2">
                <Label>Product Code *</Label>
                <Input value={form.code} onChange={e => handleChange('code', e.target.value)} placeholder="e.g., PRD-001" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Barcode</Label>
                <Input value={form.barcode} onChange={e => handleChange('barcode', e.target.value)} placeholder="e.g., 1234567890128" />
                {errors.barcode && <p className="text-xs text-red-500">{errors.barcode}</p>}
              </div>
              <div className="space-y-2">
                <Label>SKU</Label>
                <Input value={form.sku} onChange={e => handleChange('sku', e.target.value)} placeholder="e.g., SKU-ABC-123" />
