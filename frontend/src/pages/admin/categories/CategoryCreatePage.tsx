...
export default function CategoryCreatePage() {
  ...
  useEffect(() => {
    api.get<ApiResponse<Category[]>>('/admin/categories/all').then(res => setParentCategories(res.data.data));
  }, []);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={form.name} onChange={e => handleChange('name', e.target.value)} placeholder="e.g., Electronics" />
                {errors.name && <p className="text-xs text-red-500">{errors.name}</p>}
              </div>
              <div className="space-y-2">
                <Label>Category Code *</Label>
                <Input value={form.code} onChange={e => handleChange('code', e.target.value)} placeholder="e.g., CAT-001" />
